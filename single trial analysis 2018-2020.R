...
library(asremlPlus)
library(reshape)
data<- read.csv('All 2018-2020 data as of July 26 2020.csv', as.is=TRUE)

#exclude the Y3N and Y4N for now
rowixY3Y4<- grep(c("Y3N|Y4N"), data$studyName)
data<- data[-rowixY3Y4,]

#exclude the Aug for now becaue it requires high memory
...
for(i in 1:length(stdnms)){
  ...
  if(uvvmv=='MV'){
    ...
    mod<- mkConv(mod)
    p<- suppressWarnings(predictPlus(mod, classify = clasfy, meanLSD.type='factor.combination', LSDby = 'trait', pworkspace=64e6))
