...
bluesYld<- dfall[which(dfall$studyName %in% c("ORG_Dan_20", "ORG_Dan_19", "ORG_Scb_20")),]
...
mod<- mkConv(mod)
pscb<- predict(mod, classify = 'name:trait', present=list(c("studyName"),prwts=c(0, 0, 1)))
scbblup<- pscb$pvals
scbblup[which(scbblup$trait=='Heading.time...Julian.date..JD..'),'predicted.value']<- NA
pyld<- predict(mod, classify = 'name:trait', present=list(c("studyName"),prwts=c(1, 1, 0)))
yldblup<- pyld$pvals
